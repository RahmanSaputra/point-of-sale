		...
		$(target).datagrid($.extend({}, opts, {
			...
			onAfterEdit: function(index, row){
				...
				if (url){
					$.post(url, row, function(data){
						...
						if (opts.tree){
							var id_karyawan = row[opts.id_karyawan||'id_karyawan'];
							var t = $(opts.tree);
							var node = t.tree('find', id_karyawan);
							if (node){
								...
								t.tree('append', {
									...
									data: [{id_karyawan:id_karyawan,text:row[opts.treeTextField]}]
								});
							}
						}
			...
			onBeforeLoad: function(param){
				...
				if (opts.tree){
					...
					param[opts.treeParentField] = node ? node.id_karyawan : undefined;
				}
			}
		}));
		...
		if (opts.tree){
			$(opts.tree).tree({
				...
				onDrop: function(dest,source,point){
					var targetId = $(this).tree('getNode', dest).id_karyawan;
					$.ajax({
						url: opts.treeDndUrl,
						type:'post',
						data:{
	...
	$.fn.edatagrid.methods = {
		...
		addRow: function(jq){
			return jq.each(function(){
				...
				if (opts.tree){
					var node = $(opts.tree).tree('getSelected');
					rows[opts.editIndex][opts.treeParentField] = (node ? node.id_karyawan : 0);
				}
				...
				$.messager.confirm(opts.destroyMsg.confirm.title,opts.destroyMsg.confirm.msg,function(r){
					if (r){
						...
						if (row.isNewRecord){
							...
							if (opts.destroyUrl){
								var id_karyawan = row[opts.id_karyawan||'id_karyawan'];
								$.post(opts.destroyUrl, {id_karyawan:id_karyawan}, function(){
									if (opts.tree){
										dg.datagrid('reload');
										var t = $(opts.tree);
										var node = t.tree('find', id_karyawan);
